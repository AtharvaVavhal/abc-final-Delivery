...
| `FRONTEND_URL` | ✅** | ✅** | ✅ | Exact storefront origin (no trailing slash). Primary CORS origin (`credentials: true`, never a wildcard). Local Vite origins (`http://localhost:5173`, `:4173`, and `127.0.0.1`) are also allowed so local UI can call a Render API. Must be **same-site** with `BACKEND_URL` so the `SameSite=Strict` refresh cookie is sent (custom domain, e.g. `https://www.example.com` + `https://api.example.com`). A `*.vercel.app` frontend with a `*.onrender.com` API is cross-site and will not send the cookie. | `https://www.printforge.in` | Render service env |
